...
router.post('/upload-image', async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Accept both base64 data URLs and direct base64 strings
    ...
    // Validate it's a valid image by checking size
    if (buffer.length === 0 || buffer.length > 5 * 1024 * 1024) {
      return res.status(400).json({ message: 'Image size invalid. Must be between 1KB and 5MB' });
    }

    // For now, store as base64 in database
    // In production, consider using cloud storage (AWS S3, Cloudinary, etc.)
    const mediaUrl = `data:image/jpeg;base64,${base64String}`;
...
router.post('/upload-images', async (req: AuthRequest, res: Response) => {
  try {
    ...
    for (let i = 0; i < images.length; i++) {
      try {
        ...
        if (buffer.length === 0 || buffer.length > 5 * 1024 * 1024) {
          errors.push(`Image ${i + 1}: Size invalid`);
          continue;
        }
        ...
        errors.push(`Image ${i + 1}: Upload failed`);
      }
    }

    res.status(uploadedUrls.length > 0 ? 201 : 400).json({
      success: uploadedUrls.length > 0,
      urls: uploadedUrls,
      errors: errors.length > 0 ? errors : undefined,
      message: `Uploaded ${uploadedUrls.length}/${images.length} images successfully`
    });
  } catch (error) {
